...
public class Mesh {
    ...
    public Mesh(float[] VertexDataIn, int[] IndicesIn, boolean HoldInit) {
        VertexData = VertexDataIn;
        Indices = IndicesIn;
    }

    public Mesh(ArrayList<Float> VertexDataIn, ArrayList<Integer> IndicesIn, boolean HoldInit) {
        VertexData = new float[VertexDataIn.size()];
        for (int i = 0; i < VertexDataIn.size(); i++) {
            VertexData[i] = VertexDataIn.get(i);
        }
    ...
    public void Instance() {
        FloatBuffer InstanceDataBuffer = MemoryUtil.memAllocFloat(3000000);
        ((Buffer)InstanceDataBuffer.put(InstanceData)).flip();
        InstanceDataBuffer.limit(3000000);
